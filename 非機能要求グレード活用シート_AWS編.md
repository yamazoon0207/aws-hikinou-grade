...
* 365日24時間のサービス稼働となる
    * メンテナンス時間中はサービスを停止する
    * メンテナンス時間中のデータはメンテナンス後に復旧することによりデータの欠損を極力回避する

### 業務継続範囲

* 障害・メンテナンス範囲以外の箇所については極力サービスを継続する

### 目標復旧水準（業務停止時）

* 障害・メンテナンス時間中のデータはメンテナンス後に復旧することによりデータの欠損を極力回避する
...
* 冗長化方式についての詳細を以下の図に示す


## 災害対策(複数AZまたは複数リージョンの利用有無)

* サブネット構成はマルチAZとすることにより 障害時には他AZのサブネットにて運用可能とする
* xxサーバは2台を複数AZに配置し利用する
    * 運用中は1台をスタンバイとしておき アクティブ系の1台のメンテナンスや障害時には他AZのサーバに切り替える
* zzサーバについて冗長化は行わないため単一リージョン・単一AZでの利用となる
    * AZ障害時には他AZのサブネットにてバックアップからの復元を行う
* WEBサーバは2台を複数AZに配置し利用する
    * ELB配下に配置する
...
* zzサーバのEC2基盤障害時には Auto Recovery を利用し 10〜20分程度での回復となる見込み
    * AZ障害時には他AZのサブネットにてバックアップからの復元を行う時間となる
...
* 3ヶ月
    * zz サーバ に蓄積するログ容量としては以下となる
        ...
        * 1ヶ月あたり 22.5 GB
        * 3ヶ月あたり67.5 GB
        *  **4ヶ月あたり 90 GB** ※ バッチ処理の関係上 4ヶ月分がDr.Sum サーバ に蓄積される見込み
    * xxサーバにデータは蓄積しない

## 性能目標値
### オンライン

* 上に記載した通常時の業務処理量の処理を実行可能な性能とする

### バッチ

* 保管期限の過ぎた3ヶ月分のログを削除するバッチ処理が動作すること
    * バッチ処理はzzサーバのOS上で起動しOS上のログを削除する

## リソース拡張性

* EC2 はインスタンスタイプ変更によりスケールアップを行う
* EBS はボリュームを拡張または増設する


## 性能テスト

*   [Amazon EC2 Testing Policy](https://aws.amazon.com/jp/ec2/testing/) にある以下の条件を満たさないため性能試験用のAWSアカウントは不要とする
...
* EC2 と FSx は 1日 1回 バックアップ を行う  

## 運用監視

各EC2内に存在する以下のサービスを監視する (顧客依頼によりサービスの停止・起動を行う)
...
* EC2は所属するVPCに存在する NTPサーバを用いて時刻同期を行うため設定等は不要
...
* 障害時には対象サービスの再起動またはOS再起動を用いてEC2を復旧する
